...
class ClassRoom extends Component {
    ...
    constructor() {
        super();
        this.state = { classRoom: 0, students: [], teachers: [], currentClassName: "Class", loading: true}
    }
    ...
    getClassRoom(classRoom) {
        axios.post('/api/classrooms', {'action': 'info', 'classRoom': classRoom}, {headers: { Authorization: `Bearer ${sessionStorage.getItem('userToken')}` }
        }).then(res => {
            this.setState({ classRoom: classRoom, students: res.data.students, teachers: res.data.teachers, currentClassName: res.data.className, loading: false })
                ...
                {loading ? ( <Loader />) : (
                    <div>
                        <div className="avo-page-title">
                            {currentClassName} 
                            <Link to={`/classroom/${classRoom}/update`} className="title-update"><Icon icon="solar:settings-bold" /> Update Class</Link>
                        </div>
                        
                        <div className="avo-list-wrapper list-users">
                            <div className="avo-list-title">{teacherCount} {teacherTitle}</div>
                            <div className="avo-list">
                                {Object.keys(teachers).map((key, i) => (
                                    <div className="list-item" key={key}>
                                        ...
                                        <span className="list-item-large">{teachers[key].label}</span>
                                    </div>
                                ))}
                            </div>
                        </div>
